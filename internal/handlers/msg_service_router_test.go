...
func TestMsgService(t *testing.T) {
	...
	t.Run("tx without fee on msg type", func(t *testing.T) {
		...
		require.NoError(t, err, "SignTxAndGetBytes no fee")
		res := app.DeliverTx(abci.RequestDeliverTx{Tx: txBytes})
		require.Equal(t, abci.CodeTypeOK, res.Code, "res=%+v", res, "code")
		assert.Equal(t, 13, len(res.Events), "event count")
		assert.Equal(t, "tx", res.Events[4].Type, "event 4 type")
		assert.Equal(t, "fee", string(res.Events[4].Attributes[0].Key), "event 4 attr 0 key")
		assert.Equal(t, "150atom", string(res.Events[4].Attributes[0].Value), "event 4 attr 0 value")
	})

	msgbasedFee := msgfeestypes.NewMsgFee(msgTypeUrl, sdk.NewCoin("hotdog", sdk.NewInt(800)))
	require.NoError(t, app.MsgFeesKeeper.SetMsgFee(ctx, msgbasedFee), "setting fee hotdog800")

	t.Run("tx with 800hotdog fee", func(t *testing.T) {
		...
		acct := app.AccountKeeper.GetAccount(ctx, acct1.GetAddress()).(*authtypes.BaseAccount)
		txBytes, err := SignTxAndGetBytes(NewTestGasLimit(), fees, encCfg, priv.PubKey(), priv, *acct, ctx.ChainID(), msg)
		require.NoError(t, err, "SignTxAndGetBytes with fee")
		res := app.DeliverTx(abci.RequestDeliverTx{Tx: txBytes})
		require.Equal(t, abci.CodeTypeOK, res.Code, "res=%+v", res, "code")
		assert.Equal(t, 16, len(res.Events), "event count")
		assert.Equal(t, "tx", res.Events[4].Type, "event 4 type")
		assert.Equal(t, "fee", string(res.Events[4].Attributes[0].Key), "event 4 attr 0 key")
		assert.Equal(t, "150atom,800hotdog", string(res.Events[4].Attributes[0].Value), "event 4 attr 0 value")
		assert.Equal(t, "tx", res.Events[5].Type, "event 5 type")
		assert.Equal(t, "acc_seq", string(res.Events[5].Attributes[0].Key), "event 5 attr 0 key")
		assert.Equal(t, "tx", res.Events[6].Type, "event 6 type")
		assert.Equal(t, "signature", string(res.Events[6].Attributes[0].Key), "event 6 attr 0 key")
		assert.Equal(t, "tx", res.Events[13].Type, "event 13 type")
		assert.Equal(t, antewrapper.AttributeKeyAdditionalFee, string(res.Events[13].Attributes[0].Key), "event 13 attr 0 key")
		assert.Equal(t, "800hotdog", string(res.Events[13].Attributes[0].Value), "event 13 attr 0 value")
		assert.Equal(t, "tx", res.Events[14].Type, "event 14 type")
		assert.Equal(t, antewrapper.AttributeKeyBaseFee, string(res.Events[14].Attributes[0].Key) < "event 14 attr 0 key")
		assert.Equal(t, "150atom", string(res.Events[14].Attributes[0].Value), "event 14 attr 0 value")
		assert.Equal(t, "provenance.msgfees.v1.EventMsgFees", res.Events[15].Type, "event 15 type")
		assert.Equal(t, "msg_fees", string(res.Events[15].Attributes[0].Key), "event 15 attr 0 key")
		assert.Equal(t, "[{\"msg_type\":\"/cosmos.bank.v1beta1.MsgSend\",\"count\":\"1\",\"total\":\"800hotdog\",\"recipient\":\"\"}]", string(res.Events[15].Attributes[0].Value), "event 15 attr 0 value")
	})

	msgbasedFee = msgfeestypes.NewMsgFee(msgTypeUrl, sdk.NewInt64Coin("atom", 10))
	require.NoError(t, app.MsgFeesKeeper.SetMsgFee(ctx, msgbasedFee), "setting fee atom10")

	t.Run("tx with 10atom fee", func(t *testing.T) {
		msg := banktypes.NewMsgSend(addr, addr2, sdk.NewCoins(sdk.NewCoin("hotdog", sdk.NewInt(50))))
	...
	msgbasedFee = msgfeestypes.NewMsgFee(msgTypeUrl, sdk.NewInt64Coin(msgfeestypes.NhashDenom, 10))
	require.NoError(t, app.MsgFeesKeeper.SetMsgFee(ctx, msgbasedFee), "setting fee nhash10")
	require.NoError(t, testutil.FundAccount(app.BankKeeper, ctx, acct1.GetAddress(), sdk.NewCoins(sdk.NewCoin(msgfeestypes.NhashDenom, sdk.NewInt(290500010)))), "funding account")

	t.Run("tx with 10nhash fee", func(t *testing.T) {
	...
	msgbasedFee = msgfeestypes.NewMsgFee(msgTypeUrl, sdk.NewInt64Coin("atom", 100))
	require.NoError(t, app.MsgFeesKeeper.SetMsgFee(ctx, msgbasedFee), "setting fee atom100")
	require.NoError(t, testutil.FundAccount(app.BankKeeper, ctx, acct1.GetAddress(), sdk.NewCoins(sdk.NewCoin(msgfeestypes.NhashDenom, sdk.NewInt(290500010)))), "funding account again")

	t.Run("tx with 100atom fee", func(t *testing.T) {
