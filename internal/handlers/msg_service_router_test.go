...
func TestMsgService(t *testing.T) {
	...
	require.Equal(t, "300500atom,900hotdog", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance := app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "100hotdog", addr2AfterBalance, "should have the new balance after funding account")
	...
	addr1AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr1).String()
	require.Equal(t, "200400atom,50hotdog", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "150hotdog", addr2AfterBalance, "should have the new balance after funding account")
	require.Equal(t, abci.CodeTypeOK, res.Code, "res=%+v", res)
	...
	// assert.Equal(t, sdk.EventTypeTx, res.Events[6].Type)
	// assert.Equal(t, "signature", string(res.Events[6].Attributes[0].Key))
	// assert.Equal(t, sdk.EventTypeTx, res.Events[13].Type)
	...
	addr1AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr1).String()
	require.Equal(t, "100289atom", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "200hotdog", addr2AfterBalance, "should have the new balance after funding account")
	require.Equal(t, abci.CodeTypeOK, res.Code, "res=%+v", res)
...
func TestMsgServiceAuthz(t *testing.T) {
	...
	app := piosimapp.SetupWithGenesisAccounts("", []authtypes.GenesisAccount{acct1, acct2, acct3}, banktypes.Balance{Address: addr1.String(), Coins: initBalance}, banktypes.Balance{Address: addr2.String(), Coins: initBalance})
	...
	addr1AfterBalance := app.BankKeeper.GetAllBalances(ctx, addr1).String()
	require.Equal(t, "401000atom,9900hotdog", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance := app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "301000atom,9200hotdog", addr2AfterBalance, "should have the new balance after funding account")
	addr3AfterBalance := app.BankKeeper.GetAllBalances(ctx, addr3).String()
	require.Equal(t, "100hotdog", addr3AfterBalance, "should have the new balance after funding account")
	...
	fees = sdk.NewCoins(sdk.NewInt64Coin("atom", 100000), sdk.NewInt64Coin("hotdog", 1600))
	acct2 = app.AccountKeeper.GetAccount(ctx, acct2.GetAddress()).(*authtypes.BaseAccount)
	...
	addr1AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr1).String()
	require.Equal(t, "401000atom,9700hotdog", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "201000atom,7600hotdog", addr2AfterBalance, "should have the new balance after funding account")
	addr3AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr3).String()
	require.Equal(t, "300hotdog", addr3AfterBalance, "should have the new balance after funding account")
	...
	// assert.Equal(t, sdk.EventTypeTx, res.Events[6].Type)
	// assert.Equal(t, "signature", string(res.Events[6].Attributes[0].Key))
	// assert.Equal(t, sdk.EventTypeTx, res.Events[18].Type)
	...
	require.Equal(t, "401000atom,9700hotdog", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "101000atom,7599hotdog", addr2AfterBalance, "should have the new balance after funding account")
	addr3AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr3).String()
	require.Equal(t, "300hotdog", addr3AfterBalance, "should have the new balance after funding account")

	// tx authz with 2 send msgs that will exhaust the fees on the second msg
	msgExec = authztypes.NewMsgExec(addr2, []sdk.Msg{msg, msg})
	fees = sdk.NewCoins(sdk.NewInt64Coin("atom", 100000), sdk.NewInt64Coin("hotdog", 1000))
	acct2 = app.AccountKeeper.GetAccount(ctx, acct2.GetAddress()).(*authtypes.BaseAccount)
	...
	require.Equal(t, "401000atom,9700hotdog", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "1000atom,6599hotdog", addr2AfterBalance, "should have the new balance after funding account")
	addr3AfterBalance = app.BankKeeper.GetAllBalances(ctx, addr3).String()
	require.Equal(t, "300hotdog", addr3AfterBalance, "should have the new balance after funding account")

	// tx contains 1 regular send and one send in authz, should fail since authz's send will exhaust supplied fees
	msgsend := banktypes.NewMsgSend(addr2, addr3, sdk.NewCoins(sdk.NewCoin("hotdog", sdk.NewInt(100))))
	msgExec = authztypes.NewMsgExec(addr2, []sdk.Msg{msg})
	fees = sdk.NewCoins(sdk.NewInt64Coin("atom", 100_000), sdk.NewInt64Coin("hotdog", 1000))
	acct2 = app.AccountKeeper.GetAccount(ctx, acct2.GetAddress()).(*authtypes.BaseAccount)
	// using the higher gas limit coming from testdata pkg, since actually tx costs more than 100000
...
func TestMsgServiceAssessMsgFee(t *testing.T) {
	...
	addr1AfterBalance := app.BankKeeper.GetAllBalances(ctx, addr1).String()
	require.Equal(t, "1000atom,1000hotdog", addr1AfterBalance, "should have the new balance after funding account")
	addr2AfterBalance := app.BankKeeper.GetAllBalances(ctx, addr2).String()
	require.Equal(t, "87500000nhash", addr2AfterBalance, "should have the new balance after funding account")

	// assert.Equal(t, 13, len(res.Events))

	// assert.Equal(t, sdk.EventTypeTx, res.Events[4].Type)
	// assert.Equal(t, sdk.AttributeKeyFee, string(res.Events[4].Attributes[0].Key))
	// assert.Equal(t, "150atom,1190500001nhash", string(res.Events[4].Attributes[0].Value))
	// assert.Equal(t, sdk.EventTypeTx, res.Events[5].Type)
	// assert.Equal(t, antewrapper.AttributeKeyMinFeeCharged, string(res.Events[5].Attributes[0].Key))
	// assert.Equal(t, "190500000nhash", string(res.Events[5].Attributes[0].Value))
	...
	// assert.Equal(t, "150atom,1015500001nhash", string(res.Events[11].Attributes[0].Value))
