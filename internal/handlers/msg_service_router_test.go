...
func TestMsgServiceAssessMsgFee(t *testing.T) {
	...
	assert.Equal(t, "1000000000nhash", string(res.Events[9].Attributes[0].Value))
	assert.Equal(t, "tx", res.Events[10].Type)
	assert.Equal(t, antewrapper.AttributeKeyBaseFee, string(res.Events[10].Attributes[0].Key))
	assert.Equal(t, "150atom,190500000nhash", string(res.Events[10].Attributes[0].Value))
	...
	assert.Equal(t, fmt.Sprintf("[{\"msg_type\":\"/provenance.msgfees.v1.MsgAssessCustomMsgFeeRequest\",\"count\":\"1\",\"total\":\"500000000nhash\",\"recipient\":\"\"},{\"msg_type\":\"/provenance.msgfees.v1.MsgAssessCustomMsgFeeRequest\",\"count\":\"1\",\"total\":\"500000000nhash\",\"recipient\":\"%s\"}]", addr2.String()), string(res.Events[11].Attributes[0].Value))
