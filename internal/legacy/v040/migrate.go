...
import (
	...
	v040gentypes "github.com/cosmos/cosmos-sdk/x/genutil/types"
...
func migrateGenutil(oldGenState v039genutil.GenesisState) *v040gentypes.GenesisState {
	return &v040gentypes.GenesisState{
		GenTxs: oldGenState.GenTxs,
	}
}

// Migrate migrates exported state from v0.39 to a v0.40 genesis state.
func Migrate(appState v040gentypes.AppMap, clientCtx client.Context) v040gentypes.AppMap {
	v039Codec := codec.NewLegacyAmino()

	// marker must be registered first to have markeraccount types available for auth module use
	v039marker.RegisterLegacyAminoCodec(v039Codec)
	...
	var authMarkers = make([]v039marker.MarkerAccount, 0)
	...
	if appState[v039auth.ModuleName] != nil {
		...
		var regular = make([]v038auth.GenesisAccount, 0)
		for _, v039Account := range authGenState.Accounts {
			switch v039Account := v039Account.(type) {
			case *v039marker.MarkerAccount:
				{
					authMarkers = append(authMarkers, *v039Account)
				}
			default:
				{
					regular = append(regular, v039Account)
				}
			}
		}
	...
	if appState[v038staking.ModuleName] != nil {
		...
		// Migrate relative source genesis application state and marshal it into
		// the respective key.
		stakingGenesis := v040staking.Migrate(stakingGenState)

		stakingGenesis.Params.HistoricalEntries = 10000
		appState[v040staking.ModuleName] = v040Codec.MustMarshalJSON(stakingGenesis)
	}
	...
	if appState[v039genutil.ModuleName] != nil {
		...
		// Migrate relative source genesis application state and marshal it into
		// the respective key.
		appState[v040gentypes.ModuleName] = v040Codec.MustMarshalJSON(migrateGenutil(genutilGenState))
	}
	...
	if appState[v039marker.ModuleName] != nil {
		var markerGenState v039marker.GenesisState
		v039Codec.MustUnmarshalJSON(appState[v039marker.ModuleName], &markerGenState)

		// append the marker accounts from the auth accounts collection.
		markerGenState.Markers = append(markerGenState.Markers, authMarkers...)

		// delete deprecated x/marker genesis state
		delete(appState, v039marker.ModuleName)

		// Migrate relative source genesis application state and marshal it into
		// the respective key.
		appState[v040marker.ModuleName] = v040Codec.MustMarshalJSON(v040marker.Migrate(markerGenState))
	}

	// Migrate x/metadata (and deprecated /x/spec)
	if appState[v039metadata.ModuleName] != nil {
		var metadataGenState v039metadata.GenesisState
		if err := json.Unmarshal(appState[v039metadata.ModuleName], &metadataGenState); err != nil {
			panic(err)
		}

		if appState[v039metadata.SpecModuleName] != nil {
			// unmarshal relative source genesis application state
			var specGenState v039metadata.SpecGenesisState
			v039Codec.MustUnmarshalJSON(appState[v039metadata.SpecModuleName], &specGenState)

			metadataGenState.Specifications = specGenState.ContractSpecRecords
			// delete deprecated x/spec genesis state
			delete(appState, v039metadata.SpecModuleName)
		}

		// delete deprecated x/metadata genesis state
		delete(appState, v039metadata.ModuleName)

		// Migrate relative source genesis application state and marshal it into
		// the respective key.
		appState[v040metadata.ModuleName] = v040Codec.MustMarshalJSON(v040metadata.Migrate(metadataGenState))
	}
