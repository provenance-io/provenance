...
func TestSelectRandomEntries(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			entries: nil,
			count:   2,
			expErr:  "cannot choose 2 entries because there are only 0",
		},
		{
			name:    "nil entries, count 3",
			entries: nil,
			count:   3,
			expErr:  "cannot choose 3 entries because there are only 0",
		},
		{
		...
		},
		{
			...
			entries: entries[2:3],
			count:   2,
			expErr:  "cannot choose 2 entries because there are only 1",
		},
		{
			name:    "one entry, count 3",
			entries: entries[:1],
			count:   3,
			expErr:  "cannot choose 3 entries because there are only 1",
		},
		{
		...
		},
		{
			...
			entries: entries[:2],
			count:   3,
			expErr:  "cannot choose 3 entries because there are only 2",
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			var err error
			testFunc := func() {
				actual, err = SelectRandomEntries(r, tc.entries, tc.count)
			}
