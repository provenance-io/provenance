...
func (s *UpgradeTestSuite) TestTokenomics2() {
	expInLog := []string{
		...
		"INF Applying new tokenomics.",
	}
	s.AssertUpgradeHandlerLogs("tokenomics2", expInLog, nil)
}

func TestTokenomicsMaps(t *testing.T) {
	tests := []struct {
		name string
		opts *tokenomicsOptions
	}{
		{
			name: "mainnet",
			opts: getMainnetTokenomicsOptions(),
		},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			for k := range tc.opts.fullMove {
				val := tc.opts.noMove[k]
				assert.False(t, val, "%q is in both fullMove and noMove.", k)
			}
			for k := range tc.opts.noMove {
				val := tc.opts.fullMove[k]
				assert.False(t, val, "%q is in both noMove and fullMove.", k)
			}
		})
	}
}

func (s *UpgradeTestSuite) TestApplyNewTokenomicsMainnet() {
	if len(AccountData) == 0 {
		s.T().Skip("No account data to load.")
	}
	...
	var err error
	testFunc := func() {
		events, err = applyNewTokenomics(s.ctx, s.app, getMainnetTokenomicsOptions())
	}
	s.Require().NotPanics(testFunc, "applyNewTokenomics")
	logOutStr := s.logBuffer.String()
	s.T().Logf("Logged output:\n%s", logOutStr)
	s.Require().NoError(err, "applyNewTokenomics")
	s.T().Logf("number of events = %d", len(events))
	// 1321682303799875
	// 1721462524933292nhash
	// 1321682303799875nhash
}
