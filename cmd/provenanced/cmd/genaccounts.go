...
func AddGenesisMsgFeeCmd(defaultNodeHome string, interfaceRegistry types.InterfaceRegistry) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "add-genesis-msg-fee <msg-url> <additional-fee> [receipt-address]",
...
that the msg-url is a valid sdk.msg and that the fee is a valid amount and coin.
	`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msgFee := msgfeetypes.NewMsgFee(msgType, additionalFee, recipient)
			if !found {
				msgFeesGenState.MsgFees = append(msgFeesGenState.MsgFees, msgFee)
			}
