...
func (s *TestSuite) TestKeeperUsingMarkers() {
	...
	err = app.AuthzKeeper.Grant(s.ctx, granterAddr, granteeAddr, &markertypes.MarkerSendAuthorization{SpendLimit: smallCoin}, now)
	s.Require().NoError(err)
...
func (s *TestSuite) TestKeeperUsingMarkersGrantExactAsExec() {
	app, addrs := s.app, s.addrs
	...
	smallCoin := sdk.NewCoins(sdk.NewInt64Coin("steakM", 2))
	...
	// make sure all auth's are deleted
	authorization, _ = app.AuthzKeeper.GetOrRevokeAuthorization(s.ctx, granteeAddr, granterAddr, markertypes.MarkerSendAuthorization{}.MethodName())
	s.Require().Nil(authorization)

	authorization, _ = app.AuthzKeeper.GetOrRevokeAuthorization(s.ctx, granterAddr, granteeAddr, markertypes.MarkerSendAuthorization{}.MethodName())
	s.Require().Nil(authorization)
}
