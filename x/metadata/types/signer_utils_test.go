...
func TestGetUsedSigners(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "one party signed by self via signer string",
			parties: pdz(pd("addr1", "addr1", "")),
			exp:     map[string]bool{addrStr("addr1"): true},
		},
		{
			name:    "one party signed by self via signer acc",
			parties: pdz(pd("addr1", "", "addr1")),
			exp:     map[string]bool{addrStr("addr1"): true},
		},
		{
			name:    "one party signed by self via both signer string and acc",
			parties: pdz(pd("addr1", "addr1", "addr1")),
			exp:     map[string]bool{addrStr("addr1"): true},
		},
		{
			name:    "one party signed by other (authz grantee) via signer string",
			parties: pdz(pd("addr1", "grantee", "")),
			exp:     map[string]bool{},
		},
		{
			name:    "one party signed by other (authz grantee) via signer acc",
			parties: pdz(pd("addr1", "", "grantee")),
			exp:     map[string]bool{},
		},
		{
		...
		{
			name:    "two parties 1st signed by self",
			parties: pdz(pd("addr1", "addr1", ""), pd("addr2", "", "")),
			exp:     map[string]bool{addrStr("addr1"): true},
		},
		{
			name:    "two parties 2nd signed by self",
			parties: pdz(pd("addr1", "", ""), pd("addr2", "addr2", "")),
			exp:     map[string]bool{addrStr("addr2"): true},
		},
		{
			name:    "two parties both signed by self",
			parties: pdz(pd("addr1", "addr1", ""), pd("addr2", "addr2", "")),
			exp:     map[string]bool{addrStr("addr1"): true, addrStr("addr2"): true},
		},
		{
			name:    "two parties both signed by same grantee",
			parties: pdz(pd("addr1", "grantee", ""), pd("addr2", "grantee", "")),
			exp:     map[string]bool{},
		},
		{
			name:    "two parties 1st signed by self, 2nd signed by 1st via authz",
			parties: pdz(pd("addr1", "addr1", ""), pd("addr2", "addr1", "")),
			exp:     map[string]bool{addrStr("addr1"): true},
		},
		{
			name: "five parties, 1 without a signer, 2 signed by self, 2 signed by grantee",
			parties: pdz(
				pd("addr1", "addr1", ""),
				pd("addr2", "", ""),
				pd("addr3", "", "grantee"),
