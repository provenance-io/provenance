...
func (s *MetadataHandlerTestSuite) TestAddContractSpecToScopeSpec() {
	...
	cases := []struct {
		...
		{
			name : "fail to add contract spec, cannot find contract spec",
			contractSpecId : unknownContractSpecId,
			scopeSpecId : sSpec.SpecificationId,
			signers : []string{s.user1},
			errorMsg : fmt.Sprintf("contract specification not found with id %s", unknownContractSpecId),
		},
		{
			name : "fail to add contract spec, cannot find scope spec",
			contractSpecId : cSpec2.SpecificationId,
			scopeSpecId : unknownScopeSpecId,
			signers : []string{s.user1},
			errorMsg : fmt.Sprintf("scope specification not found with id %s", unknownScopeSpecId),
		},
		{
			name : "fail to add contract spec, scope spec already has contract spec",
			contractSpecId : cSpec.SpecificationId,
			scopeSpecId : sSpec.SpecificationId,
			signers : []string{s.user1},
			errorMsg : fmt.Sprintf("scope spec %s already contains contract spec %s", sSpec.SpecificationId, cSpec.SpecificationId),
		},
		{
...
func (s *MetadataHandlerTestSuite) TestDeleteContractSpecFromScopeSpec() {
	...
	cases := []struct {
		...
		{
			name: "fail to delete contract spec from scope spec, cannot find contract spec",
			contractSpecId: unknownContractSpecId,
			scopeSpecId: sSpec.SpecificationId,
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("contract specification not found with id %s", unknownContractSpecId),
		...
		{
			name: "fail to delete contract spec from scope spec, cannot find scope spec",
			contractSpecId: cSpec2.SpecificationId,
			scopeSpecId: unknownScopeSpecId,
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("scope specification not found with id %s", unknownScopeSpecId),
		...
		{
			name: "should succeed to add contract spec to scope spec",
			contractSpecId: cSpec2.SpecificationId,
			scopeSpecId: sSpec.SpecificationId,
			signers: []string{s.user1},
			errorMsg: "",
		...
		{
			name: "fail to delete contract spec from scope spec, scope spec does not contain contract spec",
			contractSpecId: cSpec2.SpecificationId,
			scopeSpecId: sSpec.SpecificationId,
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("contract specification %s not found on scope specification id %s", cSpec2.SpecificationId, sSpec.SpecificationId),
		},
	}
...
func (s *MetadataHandlerTestSuite) TestAddAndDeleteScopeOwners() {
	...
	cases := []struct {
		...
		{
			name: "setup test with new scope specification",
			msg: types.NewMsgWriteScopeSpecificationRequest(*scopeSpec, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "",
		},
		{
			name: "setup test with new scope",
			msg: types.NewMsgWriteScopeRequest(*scope, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "",
		},
		{
			name: "should fail to ADD owners, msg validate basic failure",
			msg: types.NewMsgAddScopeOwnerRequest(scopeID, []types.Party{}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "invalid owners: at least one party is required",
		},
		{
			name: "should fail to ADD owners, can not find scope",
			msg: types.NewMsgAddScopeOwnerRequest(dneScopeID, []types.Party{{Address: s.user1, Role: types.PartyType_PARTY_TYPE_OWNER}}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("scope not found with id %s", dneScopeID),
		},
		{
			name: "should fail to ADD owners, validate add failure",
			msg: types.NewMsgAddScopeOwnerRequest(scopeID, []types.Party{{Address: s.user1, Role: types.PartyType_PARTY_TYPE_OWNER}}, []string{s.user1}),
			signers: []string{s.user1},
		...
		},
		{
			...
			msg: types.NewMsgAddScopeOwnerRequest(scopeID, []types.Party{{Address: s.user2, Role: types.PartyType_PARTY_TYPE_OWNER}}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "",
		},
		{
			name: "should fail to DELETE owners, msg validate basic failure",
			msg: types.NewMsgDeleteScopeOwnerRequest(scopeID, []string{}, []string{s.user1, s.user2}),
			signers: []string{s.user1},
			errorMsg: "at least one owner address is required",
		},
		{
			name: "should fail to DELETE owners, validate add failure",
			msg: types.NewMsgDeleteScopeOwnerRequest(dneScopeID, []string{s.user1}, []string{s.user1, s.user2}),
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("scope not found with id %s", dneScopeID),
		},
		{
			name: "should fail to DELETE owners, validate add failure",
			msg: types.NewMsgDeleteScopeOwnerRequest(scopeID, []string{user3}, []string{s.user1, s.user2}),
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("address does not exist in scope owners: %s", user3),
		},
		{
			name: "should successfully DELETE owners",
			msg: types.NewMsgDeleteScopeOwnerRequest(scopeID, []string{s.user2}, []string{s.user1, s.user2}),
			signers: []string{s.user1},
			errorMsg: "",
		},
	}
...
func (s *MetadataHandlerTestSuite) TestAddAndDeleteScopeDataAccess() {
	...
	cases := []struct {
		...
		},
		{
			...
			msg: types.NewMsgAddScopeDataAccessRequest(scopeID, []string{}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "data access list cannot be empty",
		},
		{
			name: "should fail to ADD address to data access, validate add failure",
			msg: types.NewMsgAddScopeDataAccessRequest(dneScopeID, []string{s.user1}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("scope not found with id %s", dneScopeID),
		},
		{
			name: "should fail to ADD address to data access, validate add failure",
			msg: types.NewMsgAddScopeDataAccessRequest(scopeID, []string{s.user1}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("address already exists for data access %s", s.user1),
		},
		{
			name: "should successfully ADD address to data access",
			msg: types.NewMsgAddScopeDataAccessRequest(scopeID, []string{s.user2}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "",
		},
		{
			name: "should fail to DELETE address from data access, msg validate basic failure",
			msg: types.NewMsgDeleteScopeDataAccessRequest(scopeID, []string{}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "data access list cannot be empty",
		},
		{
			name: "should fail to DELETE address from data access, validate add failure",
			msg: types.NewMsgDeleteScopeDataAccessRequest(dneScopeID, []string{s.user1}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("scope not found with id %s", dneScopeID),
		},
		{
			name: "should fail to DELETE address from data access, validate add failure",
			msg: types.NewMsgDeleteScopeDataAccessRequest(scopeID, []string{user3}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: fmt.Sprintf("address does not exist in scope data access: %s", user3),
		},
		{
			name: "should successfully DELETE address from data access",
			msg: types.NewMsgDeleteScopeDataAccessRequest(scopeID, []string{s.user2}, []string{s.user1}),
			signers: []string{s.user1},
			errorMsg: "",
		},
	}
