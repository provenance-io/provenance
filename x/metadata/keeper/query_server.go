...
func (k Keeper) Sessions(c context.Context, req *types.SessionsRequest) (*types.SessionsResponse, error) {
	...
	if len(req.RecordName) > 0 {
		...
		if recordAddr.Empty() {
			...
			return &retval, sdkerrors.ErrNotFound.Wrapf("record %s does not have name %s", recordAddr, req.RecordName)
		}
	}
	...
	if !recordAddr.Empty() {
		...
		switch {
		case !found:
			return &retval, sdkerrors.ErrKeyNotFound.Wrapf("record %s does not exist", recordAddr)
		case !sessionAddr.Empty():
	...
	case !scopeAddr.Empty():
		...
		if itErr != nil {
			return &retval, sdkerrors.ErrInvalidRequest.Wrapf("error getting sessions for scope with address %s", scopeAddr)
		}
	default:
		return &retval, sdkerrors.ErrInvalidRequest.Wrap("empty request parameters")
	}
...
func (k Keeper) Records(c context.Context, req *types.RecordsRequest) (*types.RecordsResponse, error) {
	...
	if len(req.Name) > 0 {
		...
		if recordAddr.Empty() {
			...
			return &retval, sdkerrors.ErrNotFound.Wrapf("record %s does not have name %s", recordAddr, req.Name)
		}
	}
