...
func (s *AuthzTestSuite) TestValidateSmartContractSigners() {
	...
	smartAcc := func(addr string) sdk.AccountI {
		return authtypes.NewBaseAccount(acc(addr), nil, 0, 0)
	}
	...
	userAcc := func(addr string) sdk.AccountI {
...
// TODO[2137]: func (s *AuthzTestSuite) TestValidateScopeValueOwnersSigners()
...
func (s *AuthzTestSuite) TestIsMarkerAddr() {
	baseAccountAddr := sdk.AccAddress("BaseAccount_________")
	baseAccount := &authtypes.BaseAccount{
		Address:       baseAccountAddr.String(),
		AccountNumber: 1,
	}
	markerAccountAddr := sdk.AccAddress("MarkerAccount_______")
	markerAccount := &markertypes.MarkerAccount{
		BaseAccount: &authtypes.BaseAccount{
			Address:       markerAccountAddr.String(),
			AccountNumber: 2,
		},
		Denom: "fancy",
	}
	authK := NewMockAuthKeeper().WithGetAccountResults(
		&GetAccountCall{Addr: baseAccountAddr, Result: baseAccount},
		&GetAccountCall{Addr: markerAccountAddr, Result: markerAccount},
	)
	kpr := s.app.MetadataKeeper
	defer kpr.SetAuthKeeper(kpr.SetAuthKeeper(authK))
	...
	tests := []struct {
		...
		},
		{
			...
			bech32:  sdk.AccAddress("unknown_account_____").String(),
			exp:     false,
			expCall: true,
		},
		{
			name:    "base account",
			bech32:  baseAccount.Address,
			exp:     false,
			expCall: true,
		},
		{
			name:    "marker account",
			bech32:  markerAccount.Address,
			exp:     true,
			expCall: true,
		},
	}
	...
	for _, tc := range tests {
		s.Run(tc.name, func() {
			expCalls := make([]string, 0, 1)
			if tc.expCall {
				expCalls = append(expCalls, tc.bech32)
			}

			authK.ClearResults()
			var actual bool
			testFunc := func() {
				actual = kpr.IsMarkerAddr(s.FreshCtx(), tc.bech32)
			}
			s.Require().NotPanics(testFunc, "IsMarkerAddr")
			s.Assert().Equal(tc.exp, actual, "result of IsMarkerAddr")

			actCalls := make([]string, len(authK.GetAccountCalls))
			for i, call := range authK.GetAccountCalls {
				if len(call.Addr) > 0 {
					actCalls[i] = call.Addr.String()
				}
			}
			s.Assert().Equal(expCalls, actCalls, "calls to GetAccount made during IsMarkerAddr")
		})
	}
}
