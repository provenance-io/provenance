...
func (s *KeeperTestSuite) TestUnionDistinct() {
	tests := []struct {
		...
		{
			name: "one set in same set out",
			inputs: [][]string{{"a", "b", "c"}},
			output: []string{"a", "b", "c"},
		},
		{
			name: "two dup sets in single entries out",
			inputs: [][]string{{"a", "b", "c"}, {"a", "b", "c"}},
			output: []string{"a", "b", "c"},
		},
		{
			name: "unique sets in combined for out",
			inputs: [][]string{{"a", "b", "c"}, {"d", "e"}},
			output: []string{"a", "b", "c", "d", "e"},
		},
		{
			name: "empty set filled set in combined for out",
			inputs: [][]string{{}, {"a", "b", "c"}},
			output: []string{"a", "b", "c"},
		},
		{
			name: "filled set empty set in combined for out",
			inputs: [][]string{{"a", "b", "c"}, {}},
			output: []string{"a", "b", "c"},
		},
		{
			name: "two sets with one common entry in combined correctly for out",
			inputs: [][]string{{"a", "b", "c"}, {"d", "a", "e"}},
			output: []string{"a", "b", "c", "d", "e"},
		},
		{
			name: "set with one entry and set with two entries in combined correctly for out",
			inputs: [][]string{{"a"}, {"a", "b"}},
			output: []string{"a", "b"},
		},
		{
			name: "set with two entries set with one entry in combined correctly for out",
			inputs: [][]string{{"a", "b"}, {"a"}},
			output: []string{"a", "b"},
		},
		{
			name: "set with dups and set with two entries in combined correctly for out",
			inputs: [][]string{{"a", "a"}, {"a", "b"}},
			output: []string{"a", "b"},
		},
	}
