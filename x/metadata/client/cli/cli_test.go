	...
	testCases := []txCmdTestCase{
		...
		},
		{
			...
			[]string{
				metadatatypes.ScopeMetadataAddress(uuid.New()).String(),
				metadatatypes.ScopeSpecMetadataAddress(uuid.New()).String(),
				fmt.Sprintf("incorrect1,%s", s.accountAddrStr),
				s.user1AddrStr,
				s.user1AddrStr,
				fmt.Sprintf("--%s=%s", flags.FlagFrom, s.accountAddrStr),
				fmt.Sprintf("--%s=true", flags.FlagSkipConfirmation),
			...
			true, fmt.Sprintf("invalid owners: invalid party \"incorrect1,%s\": invalid address \"incorrect1\": decoding bech32 failed: invalid separator index 9", s.accountAddrStr), &sdk.TxResponse{}, 0,
		},
		{
			...
			true, fmt.Sprintf("the first message signer %s is not an owner", s.accountAddrStr), &sdk.TxResponse{}, 0,
		},
		{
