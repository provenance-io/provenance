...
func (s *TestSuite) TestDeductFeesDistributions() {
	...
	s.Assert().ErrorContains(err, "spendable balance  is smaller than 10jackthecat: insufficient funds")
	...
	err = app.MsgFeesKeeper.DeductFeesDistributions(app.BankKeeper, ctx, acct, remainingCoins, feeDist)
	s.Assert().ErrorContains(err, "spendable balance  is smaller than 1jackthecat: insufficient funds")
