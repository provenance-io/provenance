...
func (s *QueryServerTestSuite) TestCalculateTxFees() {
	...
	s.Require().NoError(s.app.MsgFeesKeeper.SetMsgFee(s.ctx, types.NewMsgFee("/cosmos.bank.v1beta1.MsgSend", sendAddFee, "", types.DefaultMsgFeeSplit)))
	response, err = s.queryClient.CalculateTxFees(s.ctx.Context(), &simulateReq)
	s.Assert().NoError(err)
	s.Assert().NotNil(response)
...
func (s *QueryServerTestSuite) TestCalculateTxFeesWithAssessCustomFees() {
	...
	additionalAccessedFeesCoin = sdk.NewInt64Coin(types.NhashDenom, 100)
	s.Require().NoError(s.app.MsgFeesKeeper.SetMsgFee(s.ctx, types.NewMsgFee(sdk.MsgTypeURL(&assessCustomFeeMsg), additionalAccessedFeesCoin, "", types.DefaultMsgFeeSplit)))
	response, err = s.queryClient.CalculateTxFees(s.ctx.Context(), &simulateReq)
	s.Assert().NoError(err)
	s.Assert().NotNil(response)
