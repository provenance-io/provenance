...
func NewQuerier(keeper Keeper, legacyQuerierCdc *codec.LegacyAmino) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) ([]byte, error) {
		...
		default:
			return nil, sdkerrors.ErrUnknownRequest.Wrapf("unknown query path: %s", path[0])
		}
	}
}
...
func queryAttribute(ctx sdk.Context, path []string, _ abci.RequestQuery, keeper Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	if len(path) < 2 {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("missing account address and/or attribute name")
	}
	addr := strings.TrimSpace(path[0])
	err := types.ValidateAttributeAddress(addr)
	if err != nil {
		return nil, sdkerrors.ErrInvalidRequest.Wrapf(fmt.Sprintf("invalid account address: %v", err))
	}
	name := strings.TrimSpace(path[1])
	if name == "" {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("attribute name cannot be empty")
	}
...
func queryAttributes(ctx sdk.Context, path []string, _ abci.RequestQuery, keeper Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	if len(path) < 1 {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("missing account address")
	}
	addr := strings.TrimSpace(path[0])
	err := types.ValidateAttributeAddress(addr)
	if err != nil {
		return nil, sdkerrors.ErrInvalidRequest.Wrapf("invalid account address: %v", err)
	}
...
func scanAttributes(ctx sdk.Context, path []string, _ abci.RequestQuery, keeper Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	if len(path) < 2 {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("missing account address and/or name suffix")
	}
	addr := strings.TrimSpace(path[0])
	if addr == "" {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("account address cannot be empty")
	}
	account, err := sdk.AccAddressFromBech32(addr)
	if err != nil {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("account address must be a Bech32 string")
	}
	suffix := strings.TrimSpace(path[1])
	if suffix == "" {
		return nil, sdkerrors.ErrInvalidRequest.Wrap("attribute name suffix cannot be empty")
	}
