...
func TestAddMarkerProposal(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"add marker - valid",
			types.NewMsgAddMarkerProposal("testrestricted", sdk.NewInt(100), sdk.AccAddress{}, types.StatusActive, types.MarkerType_RestrictedCoin, []types.AccessGrant{}, true, true, ""),
			nil,
		},
		{
			"add marker - valid no governance",
			types.NewMsgAddMarkerProposal("testnogov", sdk.NewInt(100), sdk.AccAddress{}, types.StatusActive, types.MarkerType_Coin, []types.AccessGrant{}, true, false, ""),
			nil,
		},
		{
			"add marker - valid finalized",
			types.NewMsgAddMarkerProposal("pending", sdk.NewInt(100), sdk.AccAddress{}, types.StatusFinalized, types.MarkerType_Coin, []types.AccessGrant{}, true, true, "''"),
			nil,
		},
		{
			"add marker - already exists",
			types.NewMsgAddMarkerProposal("test1", sdk.NewInt(0), sdk.AccAddress{}, types.StatusActive, types.MarkerType_Coin, []types.AccessGrant{}, true, true, ""),
			fmt.Errorf("test1 marker already exists"),
		},
		{
			"add marker - invalid status",
			types.NewMsgAddMarkerProposal("test2", sdk.NewInt(100), sdk.AccAddress{}, types.StatusUndefined, types.MarkerType_Coin, []types.AccessGrant{}, true, true, ""),
